...
class adLDAPUsers {
    ...
    */
	...
	foreach($mod as $field=>$value) {
		if (is_array($value[0])) { $mod[$field] = array(); }
	}
    ...
    * @return array
    */
    public function find($includeDescription = false, $query = false, $sorted = true){
        if (!$this->adldap->getLdapBind()){ return false; }
          
        // Perform the search and grab all their details
        $filter = "(&(objectClass=user)(samaccounttype=" . adLDAP::ADLDAP_NORMAL_ACCOUNT .")(objectCategory=person)" . $query . ")";
        $fields=array("samaccountname","displayname");
        $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), $filter, $fields);
        $entries = ldap_get_entries($this->adldap->getLdapConnection(), $sr);
